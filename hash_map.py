...
class HashMap:
    ...
    def get(self, key: str) -> object:
        ...
        for i in range(0, self.buckets.length()):
            ...
            for SLnode in bucket:

                if SLnode.key == key:  # if the key is found in SLL
                    val = SLnode.value #val= that key's value

        return val

    def put(self, key: str, value: object) -> None:
        """
        Method that updates the key / value pair in the hash map. If a given key already exists in
        the hash map, its associated value is replaced with the new value. If a given key is
        ...
        for i in range(0, self.capacity):
            ...
            for SLnode in bucket:
                ...
                if SLnode.key == key:  # if the key is found in SLL
                    SLnode.value = value #update its value
                    self.buckets.set_at_index(i, bucket)
                    return

        index= self.hash_function(key) % self.capacity
        LLcopy = self.buckets.get_at_index(index)
        LLcopy.insert(key, value)
        self.buckets.set_at_index(index, LLcopy)
        self.size += 1
        return
    ...
    def table_load(self) -> float:
        ...
        """
        tableload= float(self.size/self.capacity)
        return tableload
    ...
    def resize_table(self, new_capacity: int) -> None:
        ...
        temphash = HashMap(new_capacity, self.hash_function)

        for i in range(self.capacity):

            bucket = self.buckets.get_at_index(i) #COPY: iterates through all buckets

            for node in bucket:  #rehashes
                temphash.put(node.key, node.value)

        self.buckets = temphash.buckets
